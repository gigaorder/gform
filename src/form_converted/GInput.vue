        ...
        (
            (inputType.value === 'checkbox') ?
                <g-col class="row-flex align-items-center">
                  <g-checkbox color="primary" label={props.field.tableCell ? '' : label.value}
                              v-model={internalValue.value} v-slots={{
                    'label': () =>
                        (props.inArray) && <>
                          {props.field.tableCell ? '' : label.value} (
                          <g-icon onClick={withModifiers(removeField, ['stop'])}>
                            ...
                            (inputType.value === 'multiSelect' || inputType.value === 'multiSelect:number') ?
                                ...
                                (
                                    (props.field.tableCell) ?
                                        <input type={inputType.value} v-model={internalValue.value}
                                               class="form-control"/>
                                        :
                                        <g-col class={[flex.value, paddingClass.value]}>
                                          <g-text-field v-model={internalValue.value} label={label.value}
                                                        type={inputType.value} v-slots={{
                                            'append-inner': () => <>
                                              {
                                                (props.field.addable) &&
                                                <g-icon style="opacity: 0.4"
