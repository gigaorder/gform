<script>
  ...
  function factory(props, model) {
    ...
    function genRootWrapper(children) {
      ...
      //return <v-layout row wrap fill-height={false}>{makeAddable(props.fields, children, model)}</v-layout>
    ...
    function genNode({ node, text, childrenVNodes, isLast, state, path }) {
      ...
      for (const nodeHandler of nodeHandlers) {
        if (nodeHandler.rule(node)) return nodeHandler.genNode(...arguments, model, _path);
      }

      const value = getValueFromPath(model, node, _path, path);

      return slots => genField(...arguments, value, slots)
    }
    ...
    function itemChildren(node, { isNodeRootArray, path }) {
      ...
      for (const nodeHandler of nodeHandlers) {
        if (nodeHandler.rule(node)) return nodeHandler.itemChildren(...arguments, model);
      }
    }
