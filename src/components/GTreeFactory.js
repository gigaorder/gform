...
// ROADMAP:
// - lazy
//todo: remove all Vue factor and make it generic for all use
export default function treeFactory({
  ...
  const genTree = function () {
    ...
    const treeVNodeWithoutRoot = traverse(data).map(function (node) {
      ...
      const _context = Object.assign(this, { path, isNodeRootArray, parent, parents: this.parents.filter(p => p.node._isNode_) });
      const info = itemInfo(_.cloneDeep(node), _context);
      const children = _itemChildren.value(node, Object.assign(_context, { info }));
